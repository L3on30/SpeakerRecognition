...
'''
Load_wav, use librosa to get its form
mono means the sound emanate from one position. 
'''
...
def normalize_frames(m,epsilon=1e-12): #numpy normalize_frames
...
def remove_dc_and_dither(sin, sample_rate):
	if sample_rate == 16e3:
		...
		print("Sample rate must be 16kHz or 8kHz")
		exit(1)
	...
	return sout
'''
We can set it is cos offset, about dither audio: https://www.waves.com/audio-dithering-what-you-need-to-know
'''


def get_fft_spectrum(filename, buckets):
	signal = load_wav(filename,c.SAMPLE_RATE)
	signal *= 2**15 # chunk size
